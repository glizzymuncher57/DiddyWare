...
do
	...
	function a.d()
		local b, c, d, e = a.load("c"), a.load("b"), {}, memory.Read
		function d.Read(f, g)
			if not c.OffsetsLoaded then
				return nil
			end
			return e(g.Type, f + g.Offset)
		end
		function d.ReadVector2(f, g)
			if not c.OffsetsLoaded then
				return Vector3.new(0, 0, 0)
			end
			local h, i = e(g.Type, f + g.X), e(g.Type, f + g.Y)
			return Vector3.new(h, i, 0)
		end
		function d.GetFramePosition(f)
			if not c.OffsetsLoaded then
				return nil
			end
			local g = f.Address
			return d.Read(g, b.GuiObject.AbsolutePosition)
		end
		function d.GetFrameSize(f)
			if not c.OffsetsLoaded then
				return nil
			end
			local g = f.Address
			return d.Read(g, b.GuiObject.AbsoluteSize)
		end
		function d.GetFrameCenter(f)
			if not c.OffsetsLoaded then
				return nil
			end
			local g, h = d.GetFramePosition(f), d.GetFrameSize(f)
			return g + (h / 2)
		end
		function d.GetFrameRotation(f)
			if not c.OffsetsLoaded then
				return nil
			end
			return d.Read(f.Address, b.GuiObject.Rotation)
		end
		function d.IsFrameVisible(f)
			if not c.OffsetsLoaded then
				return nil
			end
			return d.Read(f.Address, b.GuiObject.Visible)
		end
		return d
	end
	function a.e()
		...
		local b, c, d, e, f, g, h =
			{
				...
				State = { GoingToCoin = false, CoinTween = nil, LastCoinGrab = 0, LastPositionBeforeTeleport = nil, LastTeleport = 0, Target = nil },
			}, game.GetService("Workspace"), entity.GetLocalPlayer(), a.load("a"), a.load("b"), a.load("e"), a.load("g")
		local i, j =
			function(i)
