...
do
		...
		return {
			{
				...
				run = function(e, f, g)
					...
					local i = h:find_first_child("HumanoidRootPart")
					if not i then
						return
					end
					e.name = h.Name
					e.character = h
					e.position_part = i
					e.hitbox_part = h:find_first_child("Hitbox")
	...
	function a.x()
		...
		local h = function()
			local h, i = d.GetValue("Visuals Enabled"), d.GetValue("Player ESP")
			...
			for B = 1, #A do
				...
				if C.name ~= b.local_data.player.Name then
					...
					if F then
						if C.hitbox_part then
							local G = g(C.hitbox_part)
							if G then
								if k or l then
									if p == "3D" then
										local H = f(C.hitbox_part)
										draw.polyline(H, e:create_colour(r.r, r.g, r.b), true, 1.5, r.a)
										if l then
											draw.ConvexPolyFilled(H, e:create_colour(s.r, s.g, s.b), s.a)
										end
									else
										draw.rect(G.x, G.y, G.w, G.h, e:create_colour(r.r, r.g, r.b), nil, nil, r.a)
										draw.rect(
											G.x - 1,
											G.y - 1,
											G.w + 2,
								...
								if j then
									local H, I = e:get_text_size(C.name, z)
									local J, K = G.x + (G.w / 2) - (H / 2), G.y - I - 2
									draw.text_outlined(C.name, J, K, e:create_colour(q.r, q.g, q.b), z, q.a)
								end
								if o then
									local H = (C.position - b.local_data.player_position).Magnitude
									local I = "[" .. tostring(math.floor(H)) .. "m]"
									local J, K = e:get_text_size(I, z)
									local L, M = G.x + (G.w / 2) - (J / 2), G.y + G.h + 2
									draw.text_outlined(I, L, M, e:create_colour(y.r, y.g, y.b), z, y.a)
								end
								if n then
									...
									local R = e:get_text_size(Q, z)
									draw.TextOutlined(Q, N - R - 2, O + (I - M), e:create_colour(255, 255, 255), z, 255)
								end
								if m then
		...
		local h = function()
			...
			if A then
				...
				if B then
					...
					if j then
						local C, E = e:get_text_size(x.name, w)
						local F, G = B.x + (B.w / 2) - (C / 2), B.y - E - 2
						draw.text_outlined(x.name, F, G, e:create_colour(p.r, p.g, p.b), w, p.a)
					end
					if n then
						local C = (x.position - b.local_data.player_position).Magnitude
						local E = "[" .. tostring(math.floor(C)) .. "m]"
						local F, G = e:get_text_size(E, w)
						local H, I = B.x + (B.w / 2) - (F / 2), B.y + B.h + 2
						draw.text_outlined(E, H, I, e:create_colour(v.r, v.g, v.b), w, v.a)
					end
					if m then
						local C, E, F, G = 4, B.h, x.stamina, 70
						local H = math.clamp(F / G, 0, 1)
						local I, J, K = E * H, B.x + B.w + 2, B.y
						draw.rect_filled(J, K, C, E, e:create_colour(0, 0, 0), nil, 255)
						draw.gradient(
							J + 1,
							K + (E - I) + 1,
							math.max(C - 2, 1),
	...
	function a.z()
		local b, c, d, e, f =
			a.load("b"), a.load("d"), a.load("j"), a.load("c"), function(b, c)
			...
			for o = 1, #b.cached_generators do
				...
				if q < 100 then
					...
					if t then
						...
						local x, z = "", 0
						if h then
							local A = b.local_data.player_position
							local B = f(A, p.position)
							x = "[" .. tostring(math.floor(B)) .. "m]"
							z = e:get_text_size(x, n)
						end
						local A = v + z
						local B = r - (A / 2)
						draw.text_outlined(u, B, s, e:create_colour(k.r, k.g, k.b), n, k.a)
						if h then
							draw.text_outlined(x, B + v, s, e:create_colour(i.r, i.g, i.b), n, i.a)
						end
						if l then
							local C = tostring(q) .. "%"
							local E = draw.get_text_size(C, n)
							local F, G = r - (E / 2), s + w
							draw.text_outlined(C, F, G, e:create_colour(m.r, m.g, m.b), n, m.a)
						end
					end
				end
			end
			...
			for m = 1, #b.cached_fuseboxes do
				...
				if not o then
					...
					if r then
						...
						local v, w = "", 0
						if h then
							local x = b.local_data.player_position
							local z = f(x, n.position)
							v = "[" .. tostring(math.floor(z)) .. "m]"
							w = e:get_text_size(v, l)
						end
						local x = t + w
						local z = p - (x / 2)
						draw.text_outlined(s, z, q, e:create_colour(k.r, k.g, k.b), l, k.a)
						if h then
							draw.text_outlined(v, z + t, q, e:create_colour(i.r, i.g, i.b), l, i.a)
						end
					end
				end
			end
		...
		local g = function()
			...
			for m = 1, #b.cached_items do
				...
				if q then
					...
					if h then
						local v = b.local_data.player_position
