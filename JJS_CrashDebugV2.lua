...
_P = {
	genDate = "2026-03-22T00:18:14.896356200+00:00",
	cfg = "Release",
	vers = "",
}
...
function()k()end)end return b end function a.s()local b,c,d,e,f={},a.load'c',a.
load'a',a.load'b',a.load'h'local g,h,i,j,k,l,m,n,o,p,q,r,s,t,u=d:Register(
'PV_TextSizeCache',{}),d:Register('PV_NameCache',{}),d:Register('PV_ColorCache',
{}),Color3.new(0,0,0),Color3.new(1,1,1),Color3.fromRGB,draw.Rect,draw.RectFilled
,draw.ComputeConvexHull,draw.Polyline,draw.ConvexPolyFilled,draw.TextOutlined,
draw.GetTextSize,draw.GetPartCorners,utility.WorldToScreen local v,w,x,y,z=
function(v)if type(v)~='table'then return l(255,255,255)end local w,x,y=v.R or v
.r or 255,v.G or v.g or 255,v.B or v.b or 255 local z=w..','..x..','..y local A=
i[z]if A then return A end local B=l(w,x,y)i[z]=B return B end,function(v,w)
local x=v..tostring(w)local y=g[x]if y then return y[1],y[2]end local z,A=s(v,w)
g[x]={z,A}return z,A end,function(v,w)local x=h[v]if not x then x={}h[v]=x end
local y=x[w]if y then return y end local z=string.sub(v,1,w)x[w]=z return z end,
function(v,w)local x=t(v)if not x then return end for y,z in ipairs(x)do local A
,B,C=u(z)if C then w[#w+1]={A,B}end end end,function(v)for w=#v,1,-1 do v[w]=nil
end end local aa=function(A,B,C,D)if not A or not B or not C then return end if
not A.IsAlive then return end local E,F=A.Position,C.EvasiveBarColor local G,H=
v(F),C.CooldownFillColor local I,J=v(H),C.CooldownBackgroundColor local K,L=v(J)
,C.AnimationDesyncOutlineColor local M,N=v(L),C.AnimationDesyncFillColor local O
=v(N)if C.AnimationDesyncGuides and B.Exploiting then local P=B.RootPart if not
P then return end y(P,D)if#D>=3 then local Q=o(D)if Q then p(Q,M,true,2,L.a)q(Q,
O,N.a)end end z(D)end local P,Q,R=u(E)if not R then return end local S=A.
BoundingBox if not S then return end local T,U=C.Font,B.Evade if C.
DrawEvasiveBar and U then local V=math.clamp(U/50,0,1)local W,X,Y,Z=S.h*V,S.x-8,
S.y,S.h n(X-1,Y-1,7,Z+2,j,0,255)n(X,Y+Z-W,5,W,G,0,255)local _=tostring(math.
floor(V*100))..'%'local aa=w(_,T)r(_,X-aa-2,Y,k,C.Font)end local aa=B.
OrderedMoves if C.DrawCooldowns and aa then local V=#aa local W,X,Y=S.h/V,S.x+S.
w+2,S.y for Z=1,V do local _=aa[Z]local ab,ac=_.Remaining,Y+(Z-1)*W n(X,ac,W,W,K
,0,J.a)if ab>0 then local ad=ab/_.Cooldown local ae=W*ad n(X+W-ae,ac,ae,W,I,0,H.
a)end m(X,ac,W,W,k)local ad=_.Name local ae,af=ad,T if W<40 then af=
...
Initialise()ag:Initialise()ah:Initialise()ai:Initialise()aj:Initialise()b:
Initialise()end return aa end function a.z()local aa,ab,ac={},'Combat_DiddyWare'
