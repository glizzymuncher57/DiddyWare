...
_P = {
	genDate = "2026-02-01T21:40:51.589224400+00:00",
	cfg = "Release",
	vers = "",
}
...
h:Visible(b.Enabled)end)end return b end function a.f()local b,c,d,e,f,g,h={
LastRebuild=0,LastCooldownUpdate=0},game.GetService'Players',a.load'a',entity.
GetLocalPlayer(),{},{},function(b,c,d)local e=b:GetAttribute(c)return e and e.
Value or d end local i=function(i,j)if not i or not i.Name then return end local
k,l=c:FindFirstChild(i.Name),i:GetBoneInstance'HumanoidRootPart'if not l then
return end local m=l.Parent if not m then return end local n=m:FindFirstChild
'Moveset'if not n then return end local o=d.Players[i.Name]local p,q,r=o and o.
Moves or nil,{},{}for s,t in pairs(n:GetChildren())do local u,v,w=t.Name,h(t,
'Key',1),t.Value or 0 local x,y=p and p[u],i.UserId..':'..u local z={Name=u,Key=
v,Cooldown=w,Instance=t,MoveKey=y,IsOnCooldown=x and x.IsOnCooldown or false,
Remaining=x and x.Remaining or 0}q[#q+1]=z r[u]=z end table.sort(q,function(s,t)
return s.Key<t.Key end)j[i.Name]={Player=i,Character=m,SelectedMoveset=h(m,
'Moveset','[???]'),Evade=h(m,'Evade',50),Ultimate=k and h(k,'Ultimate',0)or 0,
Ragdolled=h(m,'Ragdoll',0),Moves=r,OrderedMoves=q}end local j,k=function()local
j,k={},entity.GetLocalPlayer()if k then i(k,j)end for l,m in pairs(entity.
GetPlayers(false))do i(m,j)end d.Players=j end,function()local j=utility.
GetTickCount()/1000 for k,l in pairs(d.Players)do for m,n in pairs(l.Moves)do
local o=n.Instance if o then local p,q=h(o,'LastUse',0),n.MoveKey if p~=f[q]then
f[q]=p g[q]=j end local r=g[q]if r then local s=j-r if s<n.Cooldown then n.
IsOnCooldown=true n.Remaining=n.Cooldown-s else n.IsOnCooldown=false n.Remaining
=0 end else n.IsOnCooldown=false n.Remaining=0 end end end end end function b:
ReturnLocalPlayer()return d.Players[e.Name]end function b:GetPlayerRagdollState(
l)if not l then return end local m=d.Players[l.Name]if not m then return false
end local n=m.Ragdolled return n and n~=0 or false end function b:
GetPlayerUltimate(l)if not l then return end local m=d.Players[l.Name]if not m
then return 0 end return m.Ultimate or 0 end function b:DoesPlayerHaveMove(l,m)
if not l or not m then return false,nil end local n=d.Players[l.Name]if not n or
not n.Moves then return false,nil end local o=n.Moves[m]if not o then return
false,nil end return true,o end function b:IsMoveUsable(l,m)local n,o=b:
DoesPlayerHaveMove(l,m)if not n then return false end return not o.IsOnCooldown
end function b.Initialise(l)local m,n=l:SliderFloat(
...
end function a.p()return function(b)print'place holder'end end function a.q()
local b=a.load'd'local c=function(c,d)local e,f=c.BG1.Floor,c.Food if not e or
not f then print"Minigame Critical Error | Couldn't find floor or food!"return
nil end local g,h,j,k,l=b.GetFramePosition(e),b.GetFrameSize(e),(math.huge)for m
,n in pairs(f:GetChildren())do local o=b.GetFrameCenter(n)local p,q=math.abs(o.X
-d.X),g.Y-o.Y local r=n.Name=='Speed'and p>(h.X*0.45)if not r then if q<j then k
=n l=o j=q end end end return k,l end return function(d)local e=d.Screen:
FindFirstChild'Game'if not e then print'Minigame Critical | No Game UI.'return
end local f=e:FindFirstChild'Guy'local g=b.GetFrameCenter(f)local h,j=c(e,g)if
not h or not j then return end local k=j.X-g.X if math.abs(k)<0.03 then k=0 end
if k<0 then keyboard.click'a'elseif k>0 then keyboard.click'd'end end end
function a.r()local b,c,d={Enabled=false,FlightGame=false,CatchGame=false,GameUI
=nil,GameType=nil},a.load'a',{FlightGame=a.load'p',CatchGame=a.load'q'}local e,f
=function(e)local f=d[b.GameType]return f and f(e)or nil end,function(e,f,g)
local h=e:Get()b.Enabled=h if h then b.FlightGame=f:Get()b.CatchGame=g:Get()end
f:Visible(h)g:Visible(h)end function b.Initialise(g)local h,j,k=g:Checkbox(
'Minigames Enabled',false),g:Checkbox('Flight Game',false),g:Checkbox(
'Catch Game',false)cheat.Register('onUpdate',function()f(h,j,k)if not utility.
GetMenuState()and b.Enabled and b.GameUI and b.GameType and b[b.GameType]==true
then e(b.GameUI)end end)cheat.Register('onSlowUpdate',function()if not b.Enabled
then return end b.GameUI=c.LocalInfo.PlayerGui:FindFirstChild'DeviceUI'if b.
GameUI then local l=b.GameUI:FindFirstChild'DeviceSystem'local m=l:
FindFirstChild'Wall'and'FlightGame'or l:FindFirstChild'Food'and'CatchGame'or nil
b.GameType=m else b.GameType=nil end end)end return b end function a.s()return
function()function math.floor(b)return b-(b%1)end function math.clamp(b,c,d)
return math.max(c,math.min(d,b))end end end end local b,c,d,e,f,g,h,j,k,l,m,n,o=
a.load'b',a.load'e',a.load'h',a.load'i',a.load'j',a.load'k',a.load'l',a.load'm',
a.load'o',a.load'f',a.load'r',a.load'g',a.load's'local p=function()local p=n.
NewTab('DiddyWare_JJS','DiddyWare')local q,r,s,t=p:Container('DiddyWare_JJSC1',
'Main',{autosize=true,next=true}),p:Container('DiddyWare_JJSC2','Visuals',{
autosize=true}),p:Container('DiddyWare_JJSC3','Settings',{autosize=true,next=
true}),p:Container('DiddyWare_JJSC4','Misc',{autosize=true})o()l.Initialise(s)k.
Initialise(s)d.Initialise(q)e.Initialise(q)c.Initialise(q)b.Initialise(q)f.
Initialise(r)h.Initialise(r)g.Initialise(r)j.Initialise(s)m.Initialise(t)end p()
