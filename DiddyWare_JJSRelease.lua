...
_P = {
	genDate = "2026-01-31T18:09:34.165209500+00:00",
	cfg = "Release",
	vers = "",
}
...
end)end return b end function a.f()local b,c,d,e,f,g,h={LastRebuild=0,
...
end)end return b end function a.g()local b,c,d,e={Enabled=false,Timings={Itadori
=0.285,Mahito=0.285},_Waiting=false,_NextPressTime=0,_WasDown=false},entity.
GetLocalPlayer(),a.load'a',a.load'f'function b.Runtime()if not b.Enabled then
return end local f=d.LocalInfo.SelectedCharacter if f~='Mahito'and f~='Itadori'
then return end local g,h=utility.GetTickCount(),keyboard.IsPressed(0x33)if not
e:DoesPlayerHaveMove(c,'Focus Strike')and not e:DoesPlayerHaveMove(c,
'Divergent Fist')then return end if h and not b._WasDown and not b._Waiting then
local i=b.Timings[f]or 0 local j,k=i*1000,d.LocalInfo.Ping or 0 j=j-k if j<0
then j=0 end b._NextPressTime=g+j b._Waiting=true end if b._Waiting and g>=b.
_NextPressTime then keyboard.Click(0x33)b._Waiting=false end b._WasDown=h end
function b.Initialise(f)local g=f:Checkbox('Auto Blackflash',false)cheat.
Register('onPaint',function()b.Runtime()end)cheat.Register('onUpdate',function()
b.Enabled=g:Get()end)end return b end function a.h()local b,c={},{}c.__index=c
function c:Get()return ui.getValue(self.TabRef,self.ContainerRef,self.Name)end
function c:Set(d)ui.setValue(self.TabRef,self.ContainerRef,self.Name,d)end
function c:Visible(d)ui.setVisibility(self.TabRef,self.ContainerRef,self.Name,d)
end local d={}d.__index=d local e=function(e,f,g)return setmetatable({TabRef=e,
ContainerRef=f,Name=g},c)end function d:Checkbox(f,g)ui.newCheckbox(self.TabRef,
self.Ref,f,g)return e(self.TabRef,self.Ref,f)end function d:SliderInt(f,g,h,i)ui
...
return setmetatable({Ref=g},f)end return b end function a.i()local b,c,d,e,f={
Enabled=false,StandingDelay=500,RagdollDelay=250,_Waiting=false,_NextPressTime=0
,_WasDown=false},entity.GetLocalPlayer(),a.load'a',a.load'f',function()local b,c
,d=utility.GetMousePos(),(math.huge)for e,f in pairs(entity.GetPlayers(false))do
local g,h,i=utility.WorldToScreen(f.Position)if i then local j,k=g-b[1],h-b[2]
local l=math.sqrt(j*j+k*k)if l<c then d=f c=l end end end return d,c end
function b.Runtime()if not b.Enabled then return end if d.LocalInfo.
SelectedCharacter~='Todo'then return end local g,h=utility.GetTickCount(),
keyboard.IsPressed'r'if h and not b._WasDown and not b._Waiting then if e:
GetPlayerUltimate(c)<4 then return end local i,j=f()if j>195 then return end if
i then local k=(c.Position-i.Position).Magnitude if k>50 then return end local l
,m=e:DoesPlayerHaveMove(c,'Brothers'),e:GetPlayerRagdollState(i)local n,o=m and
b.RagdollDelay or b.StandingDelay,d.LocalInfo.Ping or 0 n=n-o if n<0 then n=0
end if l then n=250 end b._NextPressTime=g+n b._Waiting=true end end if b.
_Waiting and g>=b._NextPressTime then mouse.Click'leftmouse'b._Waiting=false end
b._WasDown=h end function b.Initialise(g)local h=g:Checkbox('Todo Perfect Swap',
false)cheat.Register('onUpdate',function()b.Runtime()b.Enabled=h:Get()end)end
return b end function a.j()local b,c,d,e,f={Enabled=false,DrawCooldowns=false,
DrawEvasiveBar=false,CooldownFillColour=Color3.fromRGB(255,0,0),
