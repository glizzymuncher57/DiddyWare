...
_P = {
	genDate = "2026-03-05T16:32:49.934974+00:00",
	cfg = "Release",
	vers = "",
}
...
==0 then return n end local p=e('ptr',o+d.Animator.AnimationTrackList)if p==0
...
'GetAnimations'then return n end return q(r,s)end cheat.register('onUpdate',
function()if not c then return end local r=c:ReturnLocalPlayer()if not r then
return end local s=r.Humanoid if not s then return end local t=s:
get_playing_animation_tracks()for u=1,#t do local v=t[u]print(v.TimePosition,v.
Animation.AnimationId)end r.Animations=s:get_playing_animation_tracks()end)end
return k end function a.q()local b,c,d=a.load'c',memory.read,{}function d.Read(e
,f)return c(f.Type,e.Address+(f.Offset or 0x0))end function d.GetFramePosition(e
)return d.ReadVector2(e,b.GuiObject.AbsolutePosition)end function d.GetFrameSize
(e)return d.ReadVector2(e,b.GuiObject.AbsoluteSize)end function d.GetFrameCenter
(e)local f,g=d.ReadVector2(e,b.GuiObject.AbsolutePosition),d.ReadVector2(e,b.
GuiObject.AbsoluteSize)return f+(g/2)end function d.ReadVector2(e,f)local g=e.
Address return Vector3.new(c(f.Type,g+f.X),c(f.Type,g+f.Y),0)end function d.
...
'DiddyWare_JJSC4','Misc',{autosize=true})q()m.Initialise(u)n.Initialise(u)l.
Initialise(u)d.Initialise(s)f.Initialise(s)e.Initialise(s)c.Initialise(s)b.
Initialise(s)g.Initialise(t)j.Initialise(t)h.Initialise(t)k.Initialise(u)o.
Initialise(v)end r()
