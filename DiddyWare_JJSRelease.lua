...
end return g end function a.g()local b,c,d,e,f,g,h,i,j,k=a.load'b',a.load'a',a.
load'c',a.load'd',a.load'e',a.load'f',memory.read,math.floor,math.min,math.max
local l,m,n={Animators=c:Register('AMAnimators',{}),Animations=c:Register(
'AMAnimations',{}),Tracks=c:Register('AMTracks',{}),_ScanState={Queue={},Index=1
,Done=false,StoredCallback=nil}},{__index=function(l,m)local n=l.Track if m==
'TimePosition'then return i(e.Read(n,f.AnimationTrack.TimePosition)*100)/100
elseif m=='Speed'then return i(e.Read(n+f.AnimationTrack.Speed)*100)/100 elseif
m=='Looped'then return e.Read(n,f.AnimationTrack.Looped)elseif m=='WeightTarget'
then return k(0,j(1,e.Read(n+f.AnimationTrack.WeightTarget)))else return nil end
end},function(l,m,n)if b.GetValue'Debug Mode'then if not l then return end l=
tostring(l)g.AddDebugMessage('[AM]:'..l,m,n)end end local o,p,q=function()if l.
_ScanState.Done then return end l._ScanState.Queue={game.DataModel}l._ScanState.
Index=1 end,function()local o,p,q=l._ScanState.Queue,l._ScanState.Index,l.
Animations for r=1,250 do local s=o[p]if not s then d.Remove('onUpdate',l.
_ScanState.StoredCallback)n('Finished Caching Animations.','info',3000)return
end if s:IsA'Animation'then local t=e.Read(s.Address,f.Animation.AnimationId)if
t~=''then q[t]=s.Name n('Cached Animation '..tostring(t),'info',3000)end end
local t=s:GetChildren()for u=1,#t do o[#o+1]=t[u]end p=p+1 end l._ScanState.
Index=p end,function(o)local p=l.Animators[o]if p then return p end local q=o:
FindFirstChildOfClass'Animator'if not q then return 0 end p=q.Address l.
Animators[o]=p return p end function l:GetPlayingAnimationTracks(r)local s,t={},
q(r)if t==0 then n("Animator doesn't exist for "..r,'warning',1000)return s end
local u=e.Read(t,f.Animator.AnimationTrackList)if u==0 then n(
'Potentially Wrong Animation Track List Offset.','warning',1000)return s end
local v=h('pointer',u)while v~=0 and v~=u do local w=h('pointer',v+0x10)if w~=0
then local x=l.Tracks[w]if not x then local y=e.Read(w,f.AnimationTrack.
Animation)if y~=0 then local z=e.Read(y,f.Animation.AnimationId)if z~=''then x={
Track=w,Animation={AnimationId=z,Name=l.Animations[z]or'Unknown'}}setmetatable(x
,m)l.Tracks[w]=x end end end if x then s[#s+1]=x end end v=h('pointer',v)end
return s end function l:Initialise()o()l._ScanState.StoredCallback=d.Add(
'onUpdate',p)end return l end function a.h()local b=a.load'a'return{LocalPlayer=
...
Initialise()e.Add('onUpdate',function()w()end)end return o end function a.j()
...
local h=self.Indexes[d]local i=e[h]if i then local j,k,l=i:GetChildren(),self.
Selectors and self.Selectors[d],{}for m=1,#j do local n=j[m]if k then n=k(n)end
if n then l[n.Name]=n end end self.Caches[d]=l local m=self.Callbacks[d]if m
then m(d,i,j)end end h=h+1 if h>#e then h=1 end self.Indexes[d]=h self.
...
Rates[c]=d end return b end function a.k()local b,c,d,e,f={},a.load'h',a.load'c'
,a.load'j'.New(),game.GetService'Workspace'local g,h=f.Items,f.Domains function
b:Initialise()e:AddCategory('Domains',{h},2000)e:AddCategory('Items',{g},2000)e:
SetCallback('Domains',function(i,j,k)local l,m,n,o=c.LocalPlayer.Entity.Position
,(math.huge)for p,q in pairs(k)do local r=q:FindFirstChildOfClass'MeshPart'local
s=r.Position local t=(s-l).Magnitude if t<m then o=s n=q m=t end end c.
ClosestDomain.Instance=n c.ClosestDomain.Distance=m c.ClosestDomain.Position=o
end)d.Add('onUpdate',function()e:Update()c.Objects.Items=e:GetCached'Items'c.
Objects.Domains=e:GetCached'Domains'end)end return b end function a.l()local b,c
,d,e,f,g=a.load'f',a.load'a',a.load'c',a.load'b',a.load'h',a.load'i'local h,i,j,
k,l,m=f.LocalPlayer.Entity,utility.GetTickCount,keyboard.IsPressed,keyboard.
Click,{Delay=0.285,CombatState=c:Register('AutoBlackFlashState',{Waiting=false,
WasDown=false,NextPressTime=0})},function(h)if not e.GetValue'Debug Mode'then
return end b.AddDebugMessage(h,'info',1000)end local n=function()local n,o,p=i()
,j(0x33),l.CombatState if o and not p.WasDown and not p.Waiting then local q=e.
GetValue'Auto Blackflash Timing'local r=q*1000 p.NextPressTime=n+r p.Waiting=
true end if p.Waiting and n>=p.NextPressTime then k(0x33)p.Waiting=false m
'Completed Yuji/Mahito Blackflash.'end l.WasDown=o end function l:Initialise()d.
Add('onUpdate',function(...)if not e.GetValue'Auto Blackflash'then return end if
e.GetValue'Auto Blackflash Hotkey'~=true then return end if not g:
DoesPlayerHaveMove(h,'Focus Strike')and not g:DoesPlayerHaveMove(h,
'Divergent Fist')then return end n()end)end return l end function a.m()local b,c
,d,e,f,g={LastClick=0},a.load'h',a.load'b',a.load'c',utility.GetTickCount,
keyboard.Click local h,i,j,k,l=c.LocalPlayer.PlayerGui,'Auto Lawyer QTE',
'Auto Lawyer QTE Delay (ms)','QTE','QTE_PC'local m=function()if not d.GetValue(i
)then return end local m=f()if(m-b.LastClick)<d.GetValue(j)then return end if
not h then h=c.LocalPlayer.Player.PlayerGui return end local n=h:FindFirstChild(
k)local o=n and n:FindFirstChild(l)if not o then return end local p=o.Value if
not p or p==''then return end g(tostring(p):lower())b.LastClick=m end function b
:Initialise()e.Add('onUpdate',m)end return b end function a.n()local b,c,d,e,f,g
,h={},a.load'c',a.load'a',a.load'b',a.load'i',a.load'f',entity.GetLocalPlayer()
local i,j,k,l=d:Register('MahoragaTimingTable',{AnimationId=
'rbxassetid://85024950165903'}),d:Register('MahoragaState',{WasEarthquaking=
false,Waiting=false}),function(i)if not e.GetValue'Debug Mode'then return end g.
AddDebugMessage(i,'info',1000)end,function(i)local j=f:GetLocalPlayer()local k=j
.Animations if not k then return nil end for l=1,#k do if k[l].Animation.
AnimationId==i then return k[l]end end return nil end local m=function()if not e
.GetValue'Auto Mahoraga Earthquake'then return end if not f:DoesPlayerHaveMove(h
,'Earthquake')then return end local m=l(i.AnimationId)if m and not j.
WasEarthquaking and not j.Waiting then j.Waiting=true end if j.Waiting then if
not m then j.Waiting=false return end local n=m.TimePosition k(
...
.GetValue'Auto Todo Blackflash'then return end if e.GetValue
'Auto Todo Blackflash Hotkey'~=true then return end if not f:DoesPlayerHaveMove(
h,'Brute Force')then return end local n=m(j.Slide)if n and not i.WasSliding and
not i.Waiting then l'Queued Todo Blackflash - Waiting for brute force'i.Waiting=
true end if i.Waiting then local o=m(j['Brute Force'])if not n and not o then l
...
'Auto Todo Blackflash Hotkey'~=true then return end if g.LocalPlayer.Data.
...
r()local b,c,d,e,f={},a.load'c',a.load'a',a.load'b',a.load'h'local g,h,i,j,k,l,m
,n,o,p,q,r,s,t,u=d:Register('PV_TextSizeCache',{}),d:Register('PV_NameCache',{})
,d:Register('PV_ColorCache',{}),Color3.new(0,0,0),Color3.new(1,1,1),Color3.
fromRGB,draw.Rect,draw.RectFilled,draw.ComputeConvexHull,draw.Polyline,draw.
ConvexPolyFilled,draw.TextOutlined,draw.GetTextSize,draw.GetPartCorners,utility.
WorldToScreen local v,w,x,y,z=function(v)if type(v)~='table'then return l(255,
255,255)end local w,x,y=v.R or v.r or 255,v.G or v.g or 255,v.B or v.b or 255
local z=w..','..x..','..y local A=i[z]if A then return A end local B=l(w,x,y)i[z
]=B return B end,function(v,w)local x=v..tostring(w)local y=g[x]if y then return
y[1],y[2]end local z,A=s(v,w)g[x]={z,A}return z,A end,function(v,w)local x=h[v]
if not x then x={}h[v]=x end local y=x[w]if y then return y end local z=string.
sub(v,1,w)x[w]=z return z end,function(v,w)local x=t(v)if not x then return end
for y,z in ipairs(x)do local A,B,C=u(z)if C then w[#w+1]={A,B}end end end,
function(v)for w=#v,1,-1 do v[w]=nil end end local aa=function(A,B,C,D)if not A
or not B or not C then return end if not A.IsAlive then return end local E,F=A.
Position,C.EvasiveBarColor local G,H=v(F),C.CooldownFillColor local I,J=v(H),C.
CooldownBackgroundColor local K,L=v(J),C.AnimationDesyncOutlineColor local M,N=
v(L),C.AnimationDesyncFillColor local O=v(N)if C.AnimationDesyncGuides and B.
Exploiting then local P=B.RootPart if not P then return end y(P,D)if#D>=3 then
local Q=o(D)if Q then p(Q,M,true,2,L.a)q(Q,O,N.a)end end z(D)end local P,Q,R=u(E
)if not R then return end local S=A.BoundingBox if not S then return end local T
,U=C.Font,B.Evade if C.DrawEvasiveBar and U then local V=math.clamp(U/50,0,1)
local W,X,Y,Z=S.h*V,S.x-8,S.y,S.h n(X-1,Y-1,7,Z+2,j,0,255)n(X,Y+Z-W,5,W,G,0,255)
local _=tostring(math.floor(V*100))..'%'local aa=w(_,T)r(_,X-aa-2,Y,k,C.Font)end
local aa=B.OrderedMoves if C.DrawCooldowns and aa then local V=#aa local W,X,Y=S
.h/V,S.x+S.w+2,S.y for Z=1,V do local _=aa[Z]local ab,ac=_.Remaining,Y+(Z-1)*W
n(X,ac,W,W,K,0,J.a)if ab>0 then local ad=ab/_.Cooldown local ae=W*ad n(X+W-ae,ac
,ae,W,I,0,H.a)end m(X,ac,W,W,k)local ad=_.Name local ae,af=ad,T if W<40 then af=
...
end return b end function a.s()local aa,ab,ac,ad,ae={},a.load'c',a.load'b',a.
...
end return aa end function a.t()local aa,ab,ac=a.load'd',a.load'e',0 local ad,ae
...
and b-ac>25 then mouse.Click'leftmouse'ac=b end end end function a.u()local aa,
...
end end end end function a.v()local aa,ab,ac,ad=a.load'c',a.load'b',a.load'a',{}
...
end)end return ad end function a.w()local aa,ab,ac,ad,ae,af={GameUI=nil,GameType
=nil},a.load'h',a.load'c',a.load'b',a.load'f',{['Flight Game']=a.load't',[
'Catch Game']=a.load'u'}local ag=function(ag)if not ad.GetValue'Debug Mode'then
...
nil aa.GameType=d else aa.GameType=nil end end)end return aa end function a.x()
local aa,ab,ac,ad,ae,af,ag,ah,ai,aj={},a.load'l',a.load'm',a.load'n',a.load'o',a
.load'p',a.load'q',a.load'r',a.load's',a.load'w'function aa:Initialise()ab:
Initialise()ac:Initialise()ad:Initialise()ae:Initialise()af:Initialise()ag:
Initialise()ah:Initialise()ai:Initialise()aj:Initialise()end return aa end
function a.y()local aa,ab,ac={},'Combat_DiddyWare','Combat'function aa:
Initialise(ad)local ae=ad:Container(ab,ac,{autosize=true,next=true})ae:Checkbox
'Auto Blackflash'ae:KeyPicker('Auto Blackflash Hotkey',true)ae:SliderFloat(
'Auto Blackflash Timing',0,1,0.285,{Debug=true})ae:Checkbox
'Auto Todo Blackflash'ae:KeyPicker('Auto Todo Blackflash Hotkey',true)ae:
...
200,75)end return aa end function a.z()local aa,ab,ac={},'VisualsTab_DiddyWare',
'Visuals'function aa:Initialise(ad)local ae=ad:Container(ab,ac,{autosize=true})
ae:Checkbox'Visuals Enabled'local af=ae:Page('Visual Types',{'Player','World'})
local ag,ah=af:For'Player',af:For'World'ag:Checkbox'Draw Cooldowns'ag:
Colorpicker('Cooldown Fill Color',{r=255,g=0,b=0,a=180},true)ag:Colorpicker(
'Cooldown Background Color',{r=0,g=0,b=0,a=200},true)ag:Checkbox
'Draw Evasive Bar'ag:Colorpicker('Evasive Fill Color',{r=121,g=74,b=148,a=255},
true)ag:Checkbox('Draw Animation Desync Guides',false)ag:Colorpicker(
...
r=255,g=255,b=255,a=255},true)end return aa end function a.A()local aa,ab,ac={},
...
'Flight Game'ae:Checkbox'Catch Game'end return aa end function a.B()local aa,ab,
ac,ad={},a.load'v','Settings_DiddyWare','Settings'function aa:Initialise(ae)
...
a.C()local aa,ab,ac,ad,ae,af={},a.load'v',a.load'y',a.load'z',a.load'A',a.load
'B'function aa:Initialise()local ag=ab.NewTab('DiddyWare_JJS','DiddyWare')ac:
Initialise(ag)ad:Initialise(ag)ae:Initialise(ag)af:Initialise(ag)end return aa
end function a.D()return function()function math.floor(aa)return aa-(aa%1)end
...
load'x',a.load'C',a.load'c',a.load'a',a.load'D',a.load'v'local b=function()ai()
