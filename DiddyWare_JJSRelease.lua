...
return b end function a.c()return{ScreenGui_Enabled=0x644,Frame_PositionX=0x518,
Frame_PositionY=0x520,Frame_SizeX=0x538,Frame_SizeY=0x53c,Frame_Rotation=0x188}
...
end)end return b end function a.f()local b,c,d,e,f,g,h={LastRebuild=0},game.
...
o.IsOnCooldown end function b.Initialise(l)local m=l:SliderFloat(
'Rebuild Player Cache Interval (s)',0.05,1,0.15)cheat.Register('onUpdate',
function()local n,o=utility.GetTickCount(),m:Get()*1000 k()if(n-b.LastRebuild)>=
o then b.LastRebuild=n j()end end)end return b end function a.g()local b,c,d,e={
Enabled=false,Timings={Itadori=0.285,Mahito=0.285},_Waiting=false,_NextPressTime
=0,_WasDown=false},entity.GetLocalPlayer(),a.load'a',a.load'f'function b.Runtime
()if not b.Enabled then return end local f=d.LocalInfo.SelectedCharacter if f~=
'Mahito'and f~='Itadori'then return end local g,h=utility.GetTickCount(),
keyboard.IsPressed(0x33)if not e:DoesPlayerHaveMove(c,'Focus Strike')and not e:
DoesPlayerHaveMove(c,'Divergent Fist')then return end if h and not b._WasDown
and not b._Waiting then local i=b.Timings[f]or 0 local j,k=i*1000,d.LocalInfo.
Ping or 0 j=j-k if j<0 then j=0 end b._NextPressTime=g+j b._Waiting=true end if
b._Waiting and g>=b._NextPressTime then keyboard.Click(0x33)b._Waiting=false end
b._WasDown=h end function b.Initialise(f)local g=f:Checkbox('Auto Blackflash',
false)cheat.Register('onPaint',function()b.Runtime()end)cheat.Register(
'onUpdate',function()b.Enabled=g:Get()end)end return b end function a.h()local b
,c={},{}c.__index=c function c:Get()return ui.getValue(self.TabRef,self.
ContainerRef,self.Name)end function c:Set(d)ui.setValue(self.TabRef,self.
ContainerRef,self.Name,d)end function c:Visible(d)ui.setVisibility(self.TabRef,
self.ContainerRef,self.Name,d)end local d={}d.__index=d local e=function(e,f,g)
return setmetatable({TabRef=e,ContainerRef=f,Name=g},c)end function d:Checkbox(f
,g)ui.newCheckbox(self.TabRef,self.Ref,f,g)return e(self.TabRef,self.Ref,f)end
function d:SliderInt(f,g,h,i)ui.newSliderInt(self.TabRef,self.Ref,f,g,h,i)return
e(self.TabRef,self.Ref,f)end function d:SliderFloat(f,g,h,i)ui.newSliderFloat(
self.TabRef,self.Ref,f,g,h,i)return e(self.TabRef,self.Ref,f)end function d:
Dropdown(f,g,h)ui.newDropdown(self.TabRef,self.Ref,f,g,h)local i=e(self.TabRef,
self.Ref,f)i.Get=function()local j=ui.getValue(self.TabRef,self.Ref,f)return g[j
+1]end return i end function d:Multiselect(f,g)ui.newMultiselect(self.TabRef,
self.Ref,f,g)local h=e(self.TabRef,self.Ref,f)h.Get=function()local i,j=ui.
getValue(self.TabRef,self.Ref,f),{}for k,l in ipairs(i)do if l then j[g[k] ]=
...
end return b end function a.i()local b,c,d,e,f={Enabled=false,StandingDelay=500,
RagdollDelay=250,_Waiting=false,_NextPressTime=0,_WasDown=false},entity.
GetLocalPlayer(),a.load'a',a.load'f',function()local b,c,d=utility.GetMousePos()
,(math.huge)for e,f in pairs(entity.GetPlayers(false))do local g,h,i=utility.
WorldToScreen(f.Position)if i then local j,k=g-b[1],h-b[2]local l=math.sqrt(j*j+
k*k)if l<c then d=f c=l end end end return d,c end function b.Runtime()if not b.
Enabled then return end if d.LocalInfo.SelectedCharacter~='Todo'then return end
local g,h=utility.GetTickCount(),keyboard.IsPressed'r'if h and not b._WasDown
and not b._Waiting then if e:GetPlayerUltimate(c)<4 then return end local i,j=f(
)if j>195 then return end if i then local k=(c.Position-i.Position).Magnitude if
k>59.5 then return end local l,m=e:DoesPlayerHaveMove(c,'Brothers'),e:
...
Initialise(g)local h=g:Checkbox('Todo Perfect Swap',false)cheat.Register(
'onUpdate',function()b.Runtime()b.Enabled=h:Get()end)end return b end function a
.j()local b,c,d,e,f={Enabled=false,DrawCooldowns=false,DrawEvasiveBar=false,
...
function a.m()local b,c={},a.load'a'function b.Initialise(d)local e=d:Dropdown(
'Font Selection',{'ConsolasBold','SmallestPixel','Verdana','Tahoma'},1)cheat.
Register('onUpdate',function()c.Font=e:Get()end)end return b end function a.n()
...
Rates[c]=d end return b end function a.o()local b,c,d,e,f={},game.GetService
...
'n'.New(),function(l,m,n)local o=l:GetAttribute(m)return o and o.Value or n end
...
b,c,d,e,f,g,h,j,k,l,m,n=a.load'b',a.load'e',a.load'g',a.load'i',a.load'j',a.load
'k',a.load'l',a.load'm',a.load'o',a.load'f',a.load'h',a.load'p'local o=function(
)local o=m.NewTab('DiddyWare_JJS','DiddyWare')local p,q,r=o:Container(
'DiddyWare_JJSC1','Main',{autosize=true,next=true}),o:Container(
'DiddyWare_JJSC2','Visuals',{autosize=true}),o:Container('DiddyWare_JJSC3',
'Settings',{autosize=true})n()l.Initialise(r)k.Initialise(r)d.Initialise(p)e.
Initialise(p)c.Initialise(p)b.Initialise(p)f.Initialise(q)h.Initialise(q)g.
Initialise(q)j.Initialise(r)end o()
