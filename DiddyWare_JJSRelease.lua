...
HotkeyEnabled=h:Get()==true end)end return b end function a.h()local b,c,d,e,f={
Enabled=false,BindPressed=false,StandingDelay=500,RagdollDelay=250,_Waiting=
false,_NextPressTime=0,_WasDown=false},entity.GetLocalPlayer(),a.load'a',a.load
'e',function()local b,c,d=utility.GetMousePos(),(math.huge)for e,f in pairs(
...
end function a.p()local b,c=a.load'c',{}function c.GetFramePosition(d)local e=d.
Address return Vector3.new(memory.read('float',e+b.FramePosition.X_Offset),
memory.read('float',e+b.FramePosition.Y_Offset),0)end function c.GetFrameSize(d)
local e=d.Address return Vector3.new(memory.read('float',e+b.FrameSize.X_Offset)
,memory.read('float',e+b.FrameSize.Y_Offset),0)end function c.GetFrameCenter(d)
