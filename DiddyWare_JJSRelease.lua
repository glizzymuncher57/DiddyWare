...
do
	...
	function a.q()
		...
		local m = function()
			...
			for p = 1, #n do
				local q = n[p]
				if j[q.Animation.Name] then
					o[#o + 1] = q
				end
			end
			local p, q = math.huge
			for r, s in pairs(o) do
				if s.TimePosition < p then
					q = s
					p = s.TimePosition
				end
			end
		...
		local n = function()
			...
			print(n.Track, i.StoredClap and i.StoredClap.Track or "0xFart")
			if i.StoredClap ~= nil and n.Track ~= i.StoredClap.Track then
				k("leftmouse")
				return
			end
			i.StoredClap = n
