...
_P = {
	genDate = "2026-02-20T12:32:22.007282100+00:00",
	cfg = "Release",
	vers = "",
}
...
local e=entity.GetPlayers(false)for f,g in pairs(e)do coroutine.resume(coroutine
.create(function()local h=g:GetBoneInstance'HumanoidRootPart'if not h then
return end local i=h:FindFirstChild'Ratio'if not i then return end if not memory
.read('bool',i.Address+c.ScreenGui_Enabled)then return end if not b.
...
end local m=memory.Read('float',l.Address+c.FramePosition.Y_Scale)if math.abs(m-
0.3)<=0.03 and not b.PressedR then keyboard.Click'r'b.PressedR=true end end))end
end function b.Initialise(e)local f=e:Checkbox'Nanami QTE'cheat.Register(
'onPaint',b.Runtime)cheat.Register('onUpdate',function()b.Enabled=f:Get()end)end
return b end function a.e()local b,c,d,e,f,g,h={LastRebuild=0,LastCooldownUpdate
=0},game.GetService'Players',a.load'a',entity.GetLocalPlayer(),{},{},function(b,
c,d)local e=b:GetAttribute(c)return e and e.Value or d end local i=function(i,j)
if not i or not i.Name then return end local k,l=c:FindFirstChild(i.Name),i:
GetBoneInstance'HumanoidRootPart'if not l then return end local m=l.Parent if
not m then return end local n=m:FindFirstChild'Moveset'if not n then return end
local o=d.Players[i.Name]local p,q,r=o and o.Moves or nil,{},{}for s,t in pairs(
n:GetChildren())do local u,v,w=t.Name,h(t,'Key',1),t.Value or 0 local x,y=p and
p[u],i.UserId..':'..u local z={Name=u,Key=v,Cooldown=w,Instance=t,MoveKey=y,
IsOnCooldown=x and x.IsOnCooldown or false,Remaining=x and x.Remaining or 0}q[#q
+1]=z r[u]=z end table.sort(q,function(s,t)return s.Key<t.Key end)j[i.Name]={
Player=i,Character=m,SelectedMoveset=h(m,'Moveset','[???]'),Evade=h(m,'Evade',50
),Ultimate=k and h(k,'Ultimate',0)or 0,Ragdolled=h(m,'Ragdoll',0),Moves=r,
OrderedMoves=q}end local j,k=function()local j,k={},entity.GetLocalPlayer()if k
then i(k,j)end for l,m in pairs(entity.GetPlayers(false))do i(m,j)end d.Players=
j end,function()local j=utility.GetTickCount()/1000 for k,l in pairs(d.Players)
do for m,n in pairs(l.Moves)do local o=n.Instance if o then local p,q=h(o,
'LastUse',0),n.MoveKey if p~=f[q]then f[q]=p g[q]=j end local r=g[q]if r then
local s=j-r if s<n.Cooldown then n.IsOnCooldown=true n.Remaining=n.Cooldown-s
else n.IsOnCooldown=false n.Remaining=0 end else n.IsOnCooldown=false n.
Remaining=0 end end end end end function b:ReturnLocalPlayer()return d.Players[e
.Name]end function b:GetPlayerRagdollState(l)if not l then return end local m=d.
Players[l.Name]if not m then return false end local n=m.Ragdolled return n and n
~=0 or false end function b:GetPlayerUltimate(l)if not l then return end local m
=d.Players[l.Name]if not m then return 0 end return m.Ultimate or 0 end function
b:DoesPlayerHaveMove(l,m)if not l or not m then return false,nil end local n=d.
Players[l.Name]if not n or not n.Moves then return false,nil end local o=n.Moves
[m]if not o then return false,nil end return true,o end function b:IsMoveUsable(
l,m)local n,o=b:DoesPlayerHaveMove(l,m)if not n then return false end return not
o.IsOnCooldown end function b.Initialise(l)local m,n=l:SliderFloat(
'Rebuild Player Cache Interval (s)',0.05,1,0.15),l:SliderFloat(
...
function a.h()local b,c,d,e,f={Enabled=false,BindPressed=false,MaxWorldDistance=
50,MaxScreenDistance=195,StandingDelay=500,RagdollDelay=250,_Waiting=false,
_NextPressTime=0,_WasDown=false},entity.GetLocalPlayer(),a.load'a',a.load'e',
function()local b,c,d=utility.GetMousePos(),(math.huge)for e,f in pairs(entity.
GetPlayers(false))do local g,h,i=utility.WorldToScreen(f.Position)if i then
local j,k=g-b[1],h-b[2]local l=math.sqrt(j*j+k*k)if l<c then d=f c=l end end end
return d,c end function b.Runtime()if not b.Enabled then return end if not b.
BindPressed then return end if d.LocalInfo.SelectedCharacter~='Todo'then return
end local g,h=utility.GetTickCount(),keyboard.IsPressed'r'if h and not b.
_WasDown and not b._Waiting then if e:GetPlayerUltimate(c)<4 then return end
local i,j=f()if j>b.MaxScreenDistance then if d.DebugMode then print(
'Target Out Of Screen Distance Range: '..tostring(b.MaxScreenDistance))end
return end if i then local k=(c.Position-i.Position).Magnitude if k>b.
MaxWorldDistance then if d.DebugMode then print(
'Target Out Of World Distance Range: '..tostring(b.MaxWorldDistance))end return
end local l,m=e:DoesPlayerHaveMove(c,'Brothers'),e:GetPlayerRagdollState(i)local
n,o=m and b.RagdollDelay or b.StandingDelay,d.LocalInfo.Ping or 0 n=n-o if n<0
then n=0 end if l then n=250 end if d.DebugMode then print(
'Executing Todo Perfect Swap, Ragdolled: '..tostring(m)..'Delay: '..tostring(g+n
))end b._NextPressTime=g+n b._Waiting=true end end if b._Waiting and g>=b.
_NextPressTime then mouse.Click'leftmouse'b._Waiting=false if d.DebugMode then
print'Executed Todo Perfect Swap.'end end b._WasDown=h end function b.Initialise
(g)local h,i,j,k,l,m=g:Checkbox('Todo Perfect Swap',false),g:KeyPicker(
'Todo Perfect Swap Hotkey',true),g:SliderInt('Target Standing Delay',1,1000,500)
,g:SliderInt('Target Ragdolled Delay',1,1000,250),g:SliderInt(
'Max World Distance',1,200,50),g:SliderInt('Max Screen Distance',1,300,195)cheat
.Register('onUpdate',function()b.Runtime()b.Enabled=h:Get()b.BindPressed=i:Get()
==true b.StandingDelay=j:Get()b.RagdollDelay=k:Get()b.MaxWorldDistance=l:Get()b.
MaxScreenDistance=m:Get()l:Visible(b.Enabled)m:Visible(b.Enabled)j:Visible(b.
Enabled)k:Visible(b.Enabled)end)end return b end function a.i()local b,c,d,e={
Enabled=false,BindEnabled=false,Delay=0.575,_WasWoosh=false,_Waiting=true,
_NextPressTime=0},entity.GetLocalPlayer(),a.load'a',a.load'e'function b.Runtime(
)if not b.Enabled then return end if not b.BindEnabled then return end if d.
LocalInfo.SelectedCharacter~='Todo'or not e:DoesPlayerHaveMove(c,'Brute Force')
then return end local f=c:GetBoneInstance'HumanoidRootPart'if not f then return
end local g,h=f:FindFirstChild'Woosh',utility.GetTickCount()if g and not b.
_WasWoosh and not b._Waiting then local i=b.Delay or 0 local j=i*1000 b.
_NextPressTime=h+j b._Waiting=true if d.DebugMode then print(
'Executing Todo Blackflash Delay: '..tostring(b._NextPressTime))end end if b.
_Waiting and h>=b._NextPressTime then keyboard.Click(0x32)b._Waiting=false if d.
DebugMode then print'Executed Todo Blackflash.'end end b._WasWoosh=g~=nil end
function b.Initialise(f)local g,h,i=f:Checkbox('Todo Blackflash',false),f:
KeyPicker('Todo Blackflash Hotkey',true),f:SliderFloat(
'Todo Blackflash Timing (s)',0,1,0.575)cheat.Register('onUpdate',function()b.
Enabled=g:Get()b.BindEnabled=h:Get()==true b.Delay=i:Get()b.Runtime()i:Visible(b
.Enabled)end)end return b end function a.j()local b,c,d,e,f={Enabled=false,
...
return b end function a.p()local b,c=a.load'c',{}function c.GetFramePosition(d)
local e=d.Address return Vector3.new(memory.read('float',e+b.FramePosition.
X_Offset),memory.read('float',e+b.FramePosition.Y_Offset),0)end function c.
GetFrameSize(d)local e=d.Address return Vector3.new(memory.read('float',e+b.
FrameSize.X_Offset),memory.read('float',e+b.FrameSize.Y_Offset),0)end function c
...
FrameRotation)end function c.IsFrameVisible(d)return memory.read('bool',d.
Address+b.Frame_Visible)end return c end function a.q()local b,c=a.load'p',0
local d=function(d,e,f)local g,h,j=math.huge,(f.X*0.05)for k,l in pairs(d:
GetChildren())do local m,n=b.GetFrameCenter(l),b.GetFrameSize(l.Top)local o=m.X+
(n.X/2)local p=o+h>e.X if p then local q=m.X-e.X if q<g then j=l g=q end end end
return j end return function(e)if not e then return end local f=e.Screen.Game
local g,h=f.Guy,utility.GetTickCount()if b.IsFrameVisible(g.Explode)then return
end local j,k=b.GetFrameSize(f),b.GetFrameCenter(g)local l=d(f.Walls,k,j)if not
l then return end local m,n=l.Top,l.Bottom local o,p,q=b.GetFramePosition(m),b.
GetFrameSize(m),b.GetFramePosition(n)local r,s=o.Y+p.Y,q.Y local t=s-r local u,v
=r+t*0.5,t*0.1 local w=u+v if k.Y>w and h-c>25 then mouse.Click'leftmouse'c=h
end end end function a.r()local b=a.load'p'local c=function(c,d)local e,f=c.BG1.
Floor,c.Food if not e or not f then print
...
'd'end end end end function a.s()local b,c,d={Enabled=false,FlightGame=false,
CatchGame=false,GameUI=nil,GameType=nil},a.load'a',{FlightGame=a.load'q',
CatchGame=a.load'r'}local e,f=function(e)local f=d[b.GameType]return f and f(e)
...
nil b.GameType=m else b.GameType=nil end end)end return b end function a.t()
...
,o,p=a.load'b',a.load'd',a.load'g',a.load'h',a.load'i',a.load'j',a.load'k',a.
load'l',a.load'm',a.load'o',a.load'e',a.load's',a.load'f',a.load't'local q=
function()local q=o.NewTab('DiddyWare_JJS','DiddyWare')local r,s,t,u=q:
Container('DiddyWare_JJSC1','Main',{autosize=true,next=true}),q:Container(
'DiddyWare_JJSC2','Visuals',{autosize=true}),q:Container('DiddyWare_JJSC3',
'Settings',{autosize=true,next=true}),q:Container('DiddyWare_JJSC4','Misc',{
autosize=true})p()m.Initialise(t)l.Initialise(t)d.Initialise(r)f.Initialise(r)e.
Initialise(r)c.Initialise(r)b.Initialise(r)g.Initialise(s)j.Initialise(s)h.
Initialise(s)k.Initialise(t)n.Initialise(u)end q()
