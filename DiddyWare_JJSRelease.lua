...
return b end function a.c()local b={GuiObject={Position={Type='float',X=0x508,Y=
0x510},Rotation={Type='float',Offset=0x188},Size={Type='float',X=0x528,Y=0x530},
AbsoluteSize={Type='float',X=0x118,Y=0x11c},AbsolutePosition={Type='float',X=
0x110,Y=0x114},Visible={Type='bool',Offset=0x5a5}},ScreenGui={Enabled={Type=
'bool',Offset=0x4bc}},DoubleConstrainedValue={Value={Type='double',Offset=0xe0}}
,Animator={AnimationTrackList=0x620},Animation={AnimationId=0xd0},AnimationTrack
={Animation=0xd0,WeightTarget=0xf0,Speed=0xe4,TimePosition=0xe8,Looped=0xf5}}
return b end function a.d()local b,c,d={Enabled=false,CurrentAddress=nil,
PressedR=false},a.load'c',a.load'a'function b.Runtime()if d.LocalInfo.
...
.read('bool',i.Address+c.ScreenGui.Enabled)then return end if not b.
...
function a.h()local b,c,d,e,f,g={Enabled=false,BindPressed=false,_Waiting=false,
_WasClapping=false},entity.GetLocalPlayer(),a.load'a',a.load'e',{Clap1=true,
Clap2=true,Clap3=true},0.65 local h=function()local h=e:ReturnLocalPlayer()local
i=h.Animations for j=1,#i do local k=i[j]if f[k.Animation.Name]then return k end
end return nil end function b.Runtime()if not b.Enabled then return end if not b
.BindPressed then return end if d.LocalInfo.SelectedCharacter~='Todo'then return
end local i=h()if e:GetPlayerUltimate(c)<4 and not i and not b._Waiting then
return end if i and not b._WasClapping and not b._Waiting then b._Waiting=true
end if b._Waiting then if not i then b._Waiting=false elseif i.TimePosition>=g
then print'fired swap'mouse.Click'leftmouse'b._Waiting=false end end b.
_WasClapping=i~=nil end function b.Initialise(i)local j,k,l=i:Checkbox(
'Todo Perfect Swap',false),i:KeyPicker('Todo Perfect Swap Hotkey',true),i:
SliderFloat('Todo Perfect Swap Wait Time',0,1,g)cheat.Register('onUpdate',
function()b.Runtime()b.Enabled=j:Get()b.BindPressed=k:Get()==true g=l:Get()end)
end return b end function a.i()local b,c,d,e,f={Enabled=false,BindEnabled=false,
WasSliding=false,Waiting=false,BrutForceStarted=false},entity.GetLocalPlayer(),a
.load'a',a.load'e',{Slide={AnimationID='rbxassetid://100081544058065'},[
'Brute Force']={AnimationID='rbxassetid://123167492985370',TimePosition=2.9}}
local g=function(g)local h=e:ReturnLocalPlayer()local i=h.Animations if not i
then return nil end for j=1,#i do if i[j].Animation.AnimationId==g then return i
[j]end end return nil end function b.Runtime()if not b.Enabled then return end
if not b.BindEnabled then return end if d.LocalInfo.SelectedCharacter~='Todo'or
not e:DoesPlayerHaveMove(c,'Brute Force')then return end local h=g(f.Slide.
...
'Brute Force'].TimePosition=k:Get()end)end return b end function a.j()local b,c,
d,e,f={Enabled=false,DrawCooldowns=false,DrawEvasiveBar=false,CooldownFillColour
=Color3.fromRGB(255,0,0),CooldownFillAlpha=180,CooldownBackgroundColour=Color3.
fromRGB(0,0,0),CooldownBackgroundAlpha=0,EvasiveBarColour=Color3.fromRGB(121,74,
148)},a.load'a',Color3.new(0,0,0),Color3.new(1,1,1),function(b)if type(b)~=
...
DrawPlayer(g,h)if not g.IsAlive then return end local i,j,k=utility.
WorldToScreen(g.Position)if not k then return end local l=g.BoundingBox if not l
then return end if b.DrawEvasiveBar and h.Evade then local m=math.clamp(h.Evade/
50,0,1)local n,o,p,q=l.h*m,l.x-8,l.y,l.h draw.RectFilled(o-1,p-1,7,q+2,d,0,255)
draw.RectFilled(o,p+q-n,5,n,b.EvasiveBarColour,0,255)local r=tostring(math.
floor(m*100))..'%'local s=draw.GetTextSize(r,c.Font)draw.TextOutlined(r,o-s-2,p,
e,c.Font)end if b.DrawCooldowns and#h.OrderedMoves>0 then local m=#h.
OrderedMoves local n,o,p=l.h/m,l.x+l.w+2,l.y for q=1,#h.OrderedMoves do local r=
h.OrderedMoves[q]local s,t=r.Remaining,p+(q-1)*n draw.RectFilled(o,t,n,n,b.
CooldownBackgroundColour,0,b.CooldownBackgroundAlpha)if s>0 then local u=s/r.
Cooldown local v=n*u draw.RectFilled(o+n-v,t,v,n,b.CooldownFillColour,0,b.
CooldownFillAlpha)end draw.Rect(o,t,n,n,e)local u,v=r.Name,c.Font local w=u if n
<40 then v='SmallestPixel'end if n<25 then w=string.sub(u,1,1)elseif n<40 then w
=string.sub(u,1,3)end if n>=15 then local x,y=draw.GetTextSize(w,v)local z,A=o+(
n-x)/2,t+(n-y)/2 draw.TextOutlined(w,z,A,e,v)end end end end function b.Draw()if
not b.Enabled then return end for g,h in pairs(entity.GetPlayers(false))do local
j=c.Players[h.Name]if j then b.DrawPlayer(h,j)end end end function b.Initialise(
g)local h,j,k,l,m,n=g:Checkbox('Character Info',false),g:Checkbox('Cooldowns',
...
function a.k()local b,c,d={Enabled=false,TextColor=Color3.fromRGB(255,255,255),
...
=h.A end)end return b end function a.l()local b,c,d,e={Enabled=false,TextColor=
...
Enabled=g:Get()b.TextColor=e(j)b.TextAlpha=j.A end)end return b end function a.m
()local b,c={},a.load'a'function b.Initialise(d)local e,f=d:Dropdown(
'Font Selection',{'ConsolasBold','SmallestPixel','Verdana','Tahoma'},1),d:
Checkbox('Debug Mode',false)cheat.Register('onUpdate',function()c.Font=e:Get()c.
DebugMode=f:Get()end)end return b end function a.n()local b={}b.__index=b
