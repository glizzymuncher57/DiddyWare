...
_P = {
	genDate = "2026-03-07T15:58:31.313896400+00:00",
	cfg = "Release",
	vers = "",
}
...
do
	...
	function a.e()
		return {
			...
			Animator = { AnimationTrackList = { Type = "pointer", Offset = 0x620 } },
	...
	function a.f()
		...
		local g, h, i, j, k, l, m, n, o, p, q, r, s, t, u, v, w =
			{ Messages = b:Register("DM.Messages", {}) }, 20, {
				Active = false,
				OffsetX = 0,
				OffsetY = 0,
			}, { info = { label = "INFO", color = Color3.new(0.4, 0.8, 1) }, warn = { label = "WARN", color = Color3.new(1, 0.85, 0.2) }, error = { label = "ERROR", color = Color3.new(1, 0.3, 0.3) }, ok = { label = "OK", color = Color3.new(0.3, 1, 0.5) } }, Color3.new(1, 1, 1), Color3.new(0.5, 0.5, 0.5), Color3.new(0.12, 0.12, 0.14), Color3.new(0.08, 0.08, 0.1), Color3.new(0.25, 0.25, 0.3), Color3.new(0.15, 0.15, 0.18), 10, 10, 600, 16, 8, 50, b:Register("DM.TextSizeCache", {})
		local x, y, z, A =
			function(x, y)
	...
	function a.g()
		...
		local l, m, n =
			...
			},
			{
				__index = function(l, m)
					local n = l.Track
					if m == "TimePosition" then
						return i(e.Read(n, f.AnimationTrack.TimePosition) * 100) / 100
					elseif m == "Speed" then
						return i(e.Read(n + f.AnimationTrack.Speed) * 100) / 100
					elseif m == "Looped" then
						return e.Read(n, f.AnimationTrack.Looped)
					elseif m == "WeightTarget" then
						return k(0, j(1, e.Read(n + f.AnimationTrack.WeightTarget)))
					else
						return nil
					end
				end,
			...
			function(l, m, n)
				if b.GetValue("Debug Mode") then
					...
					l = tostring(l)
					g.AddDebugMessage("[AM]:" .. l, m, n)
				end
			end
		local o, p, q =
			function()
				if l._ScanState.Done then
					return
				end
				l._ScanState.Queue = { game.DataModel }
				l._ScanState.Index = 1
			end, function()
				local o, p, q = l._ScanState.Queue, l._ScanState.Index, l.Animations
				for r = 1, 250 do
					local s = o[p]
					if not s then
						d.Remove("onUpdate", l._ScanState.StoredCallback)
						n("Finished Caching Animations.", "info", 3000)
						return
					end
					if s:IsA("Animation") then
						local t = e.Read(s.Address, f.Animation.AnimationId)
						if t ~= "" then
							q[t] = s.Name
							n("Cached Animation " .. tostring(t), "info", 3000)
						end
					end
				...
				l._ScanState.Index = p
			end, function(o)
				local p = l.Animators[o]
				if p then
					return p
				end
				local q = o:FindFirstChildOfClass("Animator")
				if not q then
					return 0
				end
				p = q.Address
		...
		function l:GetPlayingAnimationTracks(r)
			local s, t = {}, q(r)
			if t == 0 then
				n("Animator doesn't exist for " .. r, "warning", 1000)
				return s
			end
			local u = e.Read(t, f.Animator.AnimationTrackList)
			if u == 0 then
				n("Potentially Wrong Animation Track List Offset.", "warning", 1000)
				return s
			end
			local v = h("pointer", u)
			while v ~= 0 and v ~= u do
				local w = h("pointer", v + 0x10)
				if w ~= 0 then
					local x = l.Tracks[w]
					if not x then
						local y = e.Read(w, f.AnimationTrack.Animation)
						if y ~= 0 then
							local z = e.Read(y, f.Animation.AnimationId)
							if z ~= "" then
								x = {
									Track = w,
									Animation = { AnimationId = z, Name = l.Animations[z] or "Unknown" },
								}
								setmetatable(x, m)
								l.Tracks[w] = x
							end
		...
		function l:Initialise()
			o()
			l._ScanState.StoredCallback = d.Add("onUpdate", p)
		end
		return l
	end
	function a.h()
	...
	function a.n()
		...
		local i, j, k, l =
			d:Register("MahoragaTimingTable", { AnimationId = "rbxassetid://85024950165903" }),
			d:Register("MahoragaState", { WasEarthquaking = false, Waiting = false }),
			function(i)
				if not e.GetValue("Debug Mode") then
					return
				end
	...
	function a.p()
		...
		local n = function()
			if not e.GetValue("Auto Todo Blackflash") then
				return
			end
			if e.GetValue("Auto Todo Blackflash Hotkey") ~= true then
				return
			end
	...
	function a.w()
		local aa, ab, ac, ad, ae, af =
			...
			a.load("b"),
			a.load("f"),
			{ ["Flight Game"] = a.load("t"), ["Catch Game"] = a.load("u") }
	...
	function a.y()
		...
		function aa:Initialise(ad)
			...
			ae:Checkbox("Auto Todo Blackflash")
			ae:KeyPicker("Auto Todo Blackflash Hotkey", true)
			ae:SliderFloat("Auto Todo Blackflash Time Position", 0, 5, 2.9, { Debug = true })
			ae:Checkbox("Auto Todo Perfect Swap")
			ae:KeyPicker("Auto Todo Perfect Swap Hotkey", true)
			ae:SliderFloat("Auto Todo Perfect Swap Time Position", 0, 1, 0.65, { Debug = true })
	...
	function a.B()
		local aa, ab, ac, ad = {}, a.load("v"), "Settings_DiddyWare", "Settings"
	...
	function a.C()
		local aa, ab, ac, ad, ae, af = {}, a.load("v"), a.load("y"), a.load("z"), a.load("A"), a.load("B")
		function aa:Initialise()
